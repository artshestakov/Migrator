...
public:
    MRDefBase(const std::string& library_path)
    {
        ...
        if (!m_Library.Load())
        {
            throw ISException(m_Library.GetErrorString());
        }
    }
    ~MRDefBase() = default;
...
public:
    ...
    bool Init(IDatabase::DatabaseType database_type)
    {
        ...
        case IDatabase::DatabaseType::SQLite:
            m_LibraryPath = ISConfig::Instance().GetString("SQLite", "PathLibrary");
            res = Init<MRDefSQLite>();
            break;

        case IDatabase::DatabaseType::PostgreSQL:
            m_LibraryPath = ISConfig::Instance().GetString("PostgreSQL", "PathLibrary");
