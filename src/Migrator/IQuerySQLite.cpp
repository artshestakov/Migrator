...
void IQuerySQLite::ClearStatement()
{
    if (m_STMT)
    {
        ...
        {
            throw MRException(MRString::StringF("cannot reset statement: %s", MRDefSingleton::Get().SQLite->sqlite3_errstr(r)));
        }

        if (int r = MRDefSingleton::Get().SQLite->sqlite3_finalize(m_STMT); r != SQLITE_OK)
        {
            throw MRException(MRString::StringF("cannot finalize statement: %s", MRDefSingleton::Get().SQLite->sqlite3_errstr(r)));
        }
...
{
    (void)index;
    throw MRException("Is not realized");
    return true;
}
//-----------------------------------------------------------------------------
bool IQuerySQLite::Prepare()
{
    ...
    {
        m_ErrorString = MRString::StringF("Cannot prepare sql query\n%s\nError: %s", m_SQL.c_str(), MRDefSingleton::Get().SQLite->sqlite3_errstr(r));
        return false;
    }
...
bool IQuerySQLite::FillParams()
{
    ...
    for (size_t i = 0; i < m_Params.size(); ++i)
    {
        ...
        {
            m_ErrorString = MRString::StringF("Parameter %zu is null", i);
            return false;
        }
        ...
        if (r != SQLITE_OK)
        {
            m_ErrorString = MRString::StringF("Cannot bind parameter %zu: %s", i, MRDefSingleton::Get().SQLite->sqlite3_errstr(r));
            return false;
        }
    }
