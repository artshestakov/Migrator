...
namespace MRTemplate
{
        ...
        bool to_lower_value = true)
    {
        ...
        if (xml_attribute) //Атрибут есть
        {
            ...
            {
                throw MRException(MRString::StringF("attribute '%s' is empty on line %d",
        ...
        else //Атрибута нет
        {
            ...
            {
                throw MRException(MRString::StringF("attribute '%s' is not specified on line %d",
                    attribute_name.c_str(), xml_element->GetLineNum()));
            }
        ...
        static auto check_type_assert = [&attribute_name, &xml_element](const tinyxml2::XMLAttribute* xml_attribute, bool ok)
        {
            ...
            {
                throw MRException(MRString::StringF("attribute '%s' has invalid type on line %d",
                    attribute_name.c_str(), xml_element->GetLineNum()));
            }
        };
        ...
        if constexpr (std::is_same_v<std::string, T>)
        {
            ...
            if (to_lower_value)
            {
                MRString::StringToLower(dest_value);
            }
        }
    ...
    bool ExistsObject(const std::vector<const T*>& v, const TMetaBase* object, const std::string& file_path, const tinyxml2::XMLElement* xml_element, std::string& error_string)
    {
        for (const TMetaBase* meta_object : v)
        {
            ...
            {
                error_string = MRString::StringF("duplicate object \"%s\" in file \"%s\" on line %d",
