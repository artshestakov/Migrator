...
{
    std::vector<MRFileSystem::ISFileInfo> files;
    std::string err_msg;
    ...
    for (const std::string& path : path_list)
    {
        ...
        if (p.has_filename())
        {
            ...
            (void)MRFileSystem::MakeFileInfo(p, fi);
            files.emplace_back(fi);
        }
        else //Это директория - вытаскиваем файлы из неё
        {
            files = MRFileSystem::DirFiles(true, path, &err_msg,
                MRFileSystem::DirFileSorting::Name, MRFileSystem::SortingOrder::Ascending,
                { "SMD" });
        }
        ...
        {
            //Что-то пошло не так
            m_ErrorString = MRString::StringF("cannot iterate path '%s': %s", path.c_str(), err_msg.c_str());
            return false;
        }

        int file_amount = 0;
        for (const MRFileSystem::ISFileInfo& file_info : files)
        {
            ...
            MR_LOG.Log("Init SMD-file \"%s\" success for %lld msec", file_info.PathFull.c_str(), MRUtils::GetTickDiff(t));
            ++file_amount;
        }

        if (!file_amount)
        {
            m_ErrorString = MRString::StringF("directory has no smd-files or invalid path: %s", path.c_str());
            return false;
        }
    }
    return true;
...
bool MRMetaData::InitFile(const std::string& file_path)
{
    ...
    if (!smd_file.good())
    {
        m_ErrorString = MRString::StringF("cannot read file '%s': %s", m_CurrentFilePath.c_str(), MRString::GetLastErrorS().c_str());
        return false;
    }
...
bool MRMetaData::InitContent(const char* content, size_t size)
{
    ...
    {
        m_ErrorString = MRString::StringF("cannot parse file '%s': %s", m_CurrentFilePath.c_str(), xml_doc.ErrorStr());
        return false;
    }
    ...
    {
        m_ErrorString = MRString::StringF("name of the main tag is not valid: %s", main_tag_name.c_str());
        return false;
    }
    ...
    if (!xml_object)
    {
        m_ErrorString = MRString::StringF("file '%s' has no elements", m_CurrentFilePath.c_str());
        return false;
    }
    ...
    do
    {
        ...
        {
            m_ErrorString = MRString::StringF("Tag \"%s\" is not support on file %d",
...
bool MRMetaData::InitExecute(tinyxml2::XMLElement* xml_execute)
{
    ...
    {
        m_ErrorString = MRString::StringF("cannot parse attribute. %s", e.what());
        return false;
    }
...
bool MRMetaData::InitTable(tinyxml2::XMLElement* xml_table)
{
    ...
    try
    {
        ...
        meta_table->PrimaryFields = MRString::StringSplit(tmp, ';');
...
bool MRMetaData::InitIndexes(TMetaTable* meta_table, tinyxml2::XMLElement* xml_first_index)
{
    do
    {
        ...
        try
        {
            ...
            meta_index->Fields = MRString::StringSplit(tmp, ';');
        ...
        {
            m_ErrorString = MRString::StringF("cannot parse attribute in file %s: %s", m_CurrentFilePath.c_str(), e.what());
            return false;
        }
        ...
        {
            m_ErrorString = MRString::StringF("Invalid field name in index \"%s\" of table \"%s\" in file %s",
...
bool MRMetaData::InitForeigns(TMetaTable* meta_table, tinyxml2::XMLElement* xml_first_foreign)
{
    do
    {
        ...
        {
            m_ErrorString = MRString::StringF("Invalid field \"%s\" in the foreign \"%s\"",
...
tinyxml2::XMLElement* MRMetaData::FindSection(tinyxml2::XMLElement* xml_sections, const std::string& section_name)
{
    while (xml_sections)
    {
        if (strcmp(xml_sections->Name(), section_name.c_str()) == 0)
        {
            ...
            {
                m_ErrorString = MRString::StringF("section '%s' is empty", section_name.c_str());
                return nullptr;
            }
        }
...
bool MRMetaData::CheckSign(const std::string& content)
{
    ...
    {
        m_ErrorString = MRString::StringF("file \"%s\" is not signed", m_CurrentFilePath.c_str());
        return false;
    }

    size_t p = content.find('\n');
    if (p == MRConstants::NPOS)
    {
        m_ErrorString = MRString::StringF("invalid file \"%s\"", m_CurrentFilePath.c_str());
        return false;
    }

    std::string header = content.substr(0, p);
    MRString::StringRemoveAllChar(header, '<');
    MRString::StringRemoveAllChar(header, '!');
    MRString::StringRemoveAllChar(header, '-');
    MRString::StringRemoveAllChar(header, '>');
    ...
    if (MRCrypt::StringToSHA(MRCrypt::SHAType::SHA512, sign_content) != header)
    {
        m_ErrorString = MRString::StringF("invalid sign of file \"%s\"", m_CurrentFilePath.c_str());
        return false;
    }
    return true;
}
...
bool MRMetaData::CheckForeigns()
{
    for (const TMetaForeign* meta_foreign : m_Foreigns)
    {
        ...
        {
            m_ErrorString = MRString::StringF("Invalid reference table \"%s\" in the foreign \"%s\"",
        ...
        {
            m_ErrorString = MRString::StringF("Invalid reference field \"%s\" in the foreign \"%s\"",
