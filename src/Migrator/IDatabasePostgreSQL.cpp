...
IDatabasePostgreSQL::IDatabasePostgreSQL(DatabaseType database_type)
    ...
    m_Host(MRConfig::Instance().GetString("PostgreSQL", "Host")),
    m_Port(MRConfig::Instance().GetInt("PostgreSQL", "Port")),
    m_DatabaseName(MRConfig::Instance().GetString("PostgreSQL", "DatabaseName")),
    m_User(MRConfig::Instance().GetString("PostgreSQL", "User")),
    m_Password(MRConfig::Instance().GetString("PostgreSQL", "Password")),
    m_SystemDBName(MRConfig::Instance().GetString("PostgreSQL", "SystemDBName")),
    m_UseSystemDB(false),
    m_DB(nullptr)
{
...
std::string IDatabasePostgreSQL::GetVersion() const
{
    ...
    return MRString::StringF("%d.%d.%d", v / 10000, (v / 100) %100, v % 100);
}
//-----------------------------------------------------------------------------
...
bool IDatabasePostgreSQL::Create()
{
    ...
    MRQuery qCreate(this, MRString::StringF("CREATE DATABASE %s WITH OWNER = %s ENCODING = 'UTF8'", m_DatabaseName.c_str(), m_User.c_str()));
    if (!qCreate.Execute())
    {
...
bool IDatabasePostgreSQL::Connect()
{
    ...
    std::string connection_string = MRString::StringF("host=%s port=%d dbname=%s user=%s password=%s connect_timeout=3 application_name=Migrator",
...
bool IDatabasePostgreSQL::TableUpdate(const TMetaTable* meta_table)
{
    ...
    while (qSelect.Next())
    {
        ...
        {
            MRQuery qDrop(this, MRString::StringF("ALTER TABLE %s DROP COLUMN %s",
    ...
    while (qSelectFields.Next())
    {
        ...
        if (field_type != meta_field->Type)
        {
            ...
            if (size_t pos = using_type.find("("); pos != MRConstants::NPOS)
            {
                using_type = MRString::StringLeft(using_type, pos);
            }

            MRQuery qAlterType(this, MRString::StringF("ALTER TABLE \"%s\" ALTER COLUMN \"%s\" TYPE %s USING(%s::%s)",
        ...
        {
            MRQuery qAlterNotNull(this, MRString::StringF("ALTER TABLE \"%s\" ALTER COLUMN \"%s\" %s NOT NULL",
...
{
    MRQuery qDrop(this, MRString::StringF("ALTER TABLE %s DROP CONSTRAINT %s RESTRICT",
