...
//-----------------------------------------------------------------------------
bool GetFileList(const ISVectorString &path_list, std::vector<MRFileSystem::ISFileInfo> &files);
//-----------------------------------------------------------------------------
int main(int argc, char** argv)
{
    ...
    for (const MRFileSystem::ISFileInfo& file_info : files)
    {
        ...
        if (!file_in.is_open())
        {
            std::cout << std::endl << MRString::GetLastErrorS() << std::endl;
            return EXIT_FAILURE;
        }
        ...
        if (std::regex_search(first_line.cbegin(), first_line.cend(), match, regexp))
        {
            ...
            sign = MRCrypt::StringToSHA(MRCrypt::SHAType::SHA512, content);
        }
        ...
        file_out << MRString::StringF("<!--%s-->", sign.c_str()) << std::endl << content;
        file_out.close();

        //Переименовываем файл в исходный (с заменой)
        if (!MRFileSystem::FileMove(tmp_path, file_info.PathFull, &error_string))
        {
            std::cout << MRString::StringF("Can't rename file from \"%s\" to \"%s\": %s",
...
bool GetFileList(const ISVectorString& path_list, std::vector<MRFileSystem::ISFileInfo>& files)
{
    ...
    for (const std::string& path : path_list)
    {
        ...
        if (ec)
        {
            std::cout << MRString::StringF("Can't check path \"%s\": %s",
        ...
        {
            auto l = MRFileSystem::DirFiles(true, path, &error_string,
                MRFileSystem::DirFileSorting::DoNotSort, MRFileSystem::SortingOrder::Ascending, { "smd" });
            if (l.empty())
            {
                std::cout << MRString::StringF("Can't read directory \"%s\": %s", path.c_str(), error_string.c_str()) << std::endl;
                return false;
            }
        ...
        else //Путь ведёт к файлу
        {
            ...
            (void)MRFileSystem::MakeFileInfo(path, fi);
